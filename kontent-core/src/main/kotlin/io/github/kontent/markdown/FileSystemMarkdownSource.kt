...
interface MarkdownSource {
    fun listAllSources(urlMappings: Map<Uri, Uri>, standalonePages: Set<PageSource>): List<MarkdownSourceFile>
...
class FileSystemMarkdownSource(private val sourcePath: ContentSourcePath) : MarkdownSource {
    ...
    override fun listAllSources(urlMappings: Map<Uri, Uri>, standalonePages: Set<PageSource>) =
        File(sourcePath.value).walkTopDown()
    ...
    override fun read(sourceFile: MarkdownSourceFile) = Markdown(File(sourceFile.location.path).readText())
}

fun File.resolvePageUri(sourcePath: ContentSourcePath) =
    Uri.of(relativePath(sourcePath)
        .removeSuffix(".md")
        .removeSuffix("/index")
        .ensureFirstSlash())
